...
class_name protoMovementComponent
extends Node

@export var can_move: bool = true
var is_crouching: bool = false
var apply_friction: bool = true
...
func handle_horizontal_movement(body: CharacterBody2D, direction: float, delta) -> void:
	...
	if dash_component.is_currently_dashing():
	...
	if direction == 0 or sign(direction) != sign(body.velocity.x):
		_apply_friction(body, friction, air_resistance)

			
	if abs(body.velocity.x) > speed:
		_apply_friction(body, 0.99, 0.99)
		
func _apply_friction(body, ground_friction, air_friction) -> void:
	#var tps = Engine.physics_ticks_per_second
	#print(delta," * ",tps,"[",(delta*tps)/(tps/60),"]")
	if apply_friction:
		var applied_friction: float = 0
		if get_parent().is_on_floor():
			applied_friction = ground_friction
		else:
			applied_friction = air_friction
		print("Applying Friction: ",body," ",applied_friction)
